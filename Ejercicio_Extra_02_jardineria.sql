...
/*Utiliza la función ADDDATE para sumar dos días a la fecha de entrega
y luego compararla con la fecha esperada.*/

#B- Utilizando la función DATEDIFF de MySQL.
SELECT codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega, estado
FROM pedido
WHERE DATEDIFF(fecha_esperada, fecha_entrega) >= 2;
/*Utiliza la función DATEDIFF para calcular la diferencia de días entre la fecha esperada
 y la fecha de entrega, y luego verifica si esa diferencia es mayor o igual a 2.*/
...
#de ventas.
SELECT nombre_cliente as 'Cliente', concat(nombre,' ',apellido1,' ',apellido2) as 'Representante'
FROM cliente
...
#representantes de ventas.
SELECT a.nombre_cliente as 'Cliente', concat(nombre,' ',apellido1,' ',apellido2) as 'Representante', c.fecha_pago, c.total
FROM cliente a
JOIN empleado b ON a.codigo_empleado_rep_ventas = b.codigo_empleado
JOIN pago c ON a.codigo_cliente = c.codigo_cliente;

#3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de
#sus representantes de ventas.
SELECT DISTINCT c.nombre_cliente, CONCAT(nombre,' ',apellido1,' ',apellido2) AS 'Representante', puesto
FROM cliente c
...
SELECT DISTINCT a.nombre_cliente as 'Cliente', concat(nombre,' ',apellido1,' ',apellido2) as 'Representante', puesto, d.ciudad
FROM cliente a
JOIN empleado b ON a.codigo_empleado_rep_ventas = b.codigo_empleado
JOIN pago c ON a.codigo_cliente = c.codigo_cliente
JOIN oficina d ON d.codigo_oficina=b.codigo_oficina;

#5. Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus
#representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT DISTINCT a.nombre_cliente as 'Cliente', concat(b.nombre,' ',b.apellido1,' ',b.apellido2) as 'Representante', b.puesto, d.ciudad
FROM cliente a
JOIN empleado b ON a.codigo_empleado_rep_ventas = b.codigo_empleado 
JOIN oficina d ON b.codigo_oficina = d.codigo_oficina 
LEFT JOIN pago c ON a.codigo_cliente = c.codigo_cliente 
WHERE c.codigo_cliente IS NULL;

SELECT DISTINCT c.nombre_cliente, e.nombre, e.apellido1, apellido2, o.ciudad as 'Ciudad oficina', puesto 
FROM cliente c 
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas=e.codigo_empleado 
...
SELECT DISTINCT a.nombre_cliente as 'Cliente', concat(b.nombre,' ',b.apellido1,' ',b.apellido2) as 'Representante', b.puesto, d.ciudad
FROM cliente a, empleado b, oficina d
WHERE a.codigo_empleado_rep_ventas = b.codigo_empleado 
  AND b.codigo_oficina = d.codigo_oficina
  AND a.codigo_cliente NOT IN (SELECT DISTINCT codigo_cliente FROM pago);
...
SELECT DISTINCT c.nombre_cliente as 'Cliente', concat(e.nombre,' ',e.apellido1,' ',e.apellido2) as 'Representante', e.puesto, o.ciudad
FROM cliente c 
INNER JOIN empleado e ON c.codigo_empleado_rep_ventas=e.codigo_empleado 
INNER JOIN oficina o ON  o.codigo_oficina = e.codigo_oficina;
...
SELECT c.nombre_cliente, p.*, DATEDIFF(IFNULL(p.fecha_entrega, CURDATE()), p.fecha_esperada) AS dias_demora
FROM cliente c
INNER JOIN pedido p ON c.codigo_cliente = p.codigo_cliente
WHERE p.fecha_entrega > p.fecha_esperada OR (p.fecha_entrega IS NULL AND p.fecha_esperada < CURDATE());
/*Esta consulta utiliza la función IFNULL() para sustituir la fecha de entrega NULL con la fecha actual,
 y luego se utiliza DATEDIFF() para calcular la diferencia en días entre las dos fechas.
 El resultado se muestra en la columna "dias_demora".*/

#10. Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
SELECT a.codigo_cliente, a.nombre_cliente, e.gama, COUNT(DISTINCT c.codigo_producto) as cantidad_productos_comprados
FROM cliente a
INNER JOIN pedido b ON a.codigo_cliente = b.codigo_cliente
INNER JOIN detalle_pedido c ON b.codigo_pedido = c.codigo_pedido
INNER JOIN producto e ON c.codigo_producto = e.codigo_producto
GROUP BY a.codigo_cliente, e.gama;

#Consultas multitabla (Composición externa)
#Resuelva todas las consultas utilizando las cláusulas LEFT JOIN, RIGHT JOIN, JOIN.
#1. Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.
SELECT DISTINCT c.nombre_cliente, p.*
FROM cliente c
...
#ningún pago.
SELECT c.codigo_cliente, c.nombre_cliente, p.*
FROM cliente c
LEFT JOIN pedido p ON c.codigo_cliente = p.codigo_cliente
...
SELECT e.codigo_empleado, e.nombre, e.apellido1, e.apellido2, j.nombre AS jefe_nombre, j.apellido1 AS jefe_apellido1, j.apellido2 AS jefe_apellido2
FROM empleado e
LEFT JOIN cliente c ON e.codigo_empleado = c.codigo_empleado_rep_ventas
...
SELECT COUNT(*) as Cant_Empleados FROM empleado;

#2. ¿Cuántos clientes tiene cada país?
SELECT pais, COUNT(codigo_cliente) as Cant_Clientes
FROM cliente
GROUP BY pais;

#3. ¿Cuál fue el pago medio en 2009?
SELECT round(avg(total)) as pago_Medio FROM pago
...
#número de pedidos.
SELECT estado, COUNT(codigo_pedido) AS cant_pedidos
FROM pedido
GROUP BY estado
...
#cada uno.
SELECT concat(nombre, ' ', apellido1, ' ',apellido2) as 'Representate Ventas', COUNT(codigo_cliente) as Cant_Clientes
FROM empleado
JOIN cliente ON codigo_empleado_rep_ventas = codigo_empleado
GROUP BY codigo_empleado_rep_ventas;

#10. Calcula el número de clientes que no tiene asignado representante de ventas.
SELECT COUNT(codigo_cliente) AS clientes_sin_rep
FROM cliente
LEFT JOIN empleado ON codigo_empleado_rep_ventas = codigo_empleado
...
SELECT DISTINCT codigo_pedido, COUNT(codigo_producto) AS Nro_Productos
FROM detalle_pedido
GROUP BY codigo_pedido;

SELECT p.codigo_pedido, COUNT(DISTINCT dp.codigo_producto) as Cant_Productos
FROM pedido p
...
#unidades vendidas.
SELECT p.codigo_producto, p.nombre, SUM(dp.cantidad) AS Cantidad
FROM producto p
...
SELECT p.codigo_producto, p.nombre, SUM(dp.cantidad) AS unidades_vendidas
...
SELECT SUM(subtotal) AS base_imponible, SUM(subtotal)*0.21 AS iva, SUM(subtotal)*1.21 AS total_facturado
FROM (
    SELECT dp.cantidad * dp.precio_unidad AS subtotal
    FROM detalle_pedido dp    
) AS ventas;
...
SELECT p.nombre, SUM(d.cantidad) AS unidades_vendidas, 
       SUM(d.cantidad * d.precio_unidad) AS total_facturado,
       SUM(d.cantidad * d.precio_unidad) * 1.21 AS total_con_iva
FROM producto p
...
SELECT p.nombre, p.codigo_producto, SUM(dp.cantidad) AS total_vendido
...
SELECT p.nombre AS nombre_producto, p.codigo_producto ,SUM(dp.cantidad) AS total_unidades_vendidas
FROM producto p
...
#INNER JOIN).
SELECT c.nombre_cliente, SUM(p.total) AS Pagado, c.limite_credito
...
AND apellido1 LIKE 'Soria' OR apellido2 LIKE 'Soria');
...
SELECT DISTINCT nombre, apellido1, puesto 
FROM empleado
...
SELECT e.nombre, e.apellido1, e.apellido2, e.puesto, o.telefono, e.extension
FROM empleado e
...
FROM cliente c
WHERE NOT EXISTS (SELECT * FROM pago p WHERE p.codigo_cliente = c.codigo_cliente);
...
SELECT *
FROM producto p
WHERE NOT EXISTS (SELECT * FROM detalle_pedido d WHERE d.codigo_producto = p.codigo_producto);

#4. Devuelve un listado de los productos que han aparecido en un pedido alguna vez.
SELECT *
FROM producto p
WHERE EXISTS (SELECT * FROM detalle_pedido d WHERE d.codigo_producto = p.codigo_producto);
